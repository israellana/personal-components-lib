module.exports = [
    'bg-primary-50',
    'bg-primary-100',
    'bg-primary-200',
    'bg-primary-300',
    'bg-primary-400',
    'bg-primary-500',
    'bg-secondary-50',
    'bg-secondary-100',
    'bg-secondary-200',
    'bg-secondary-300',
    'bg-secondary-400',
    'bg-secondary-500',
    'bg-green-50',
    'bg-green-100',
    'bg-green-200',
    'bg-green-300',
    'bg-green-400',
    'bg-green-500',
    'bg-orange-50',
    'bg-orange-100',
    'bg-orange-200',
    'bg-orange-300',
    'bg-orange-400',
    'bg-orange-500',
    'bg-red-50',
    'bg-red-100',
    'bg-red-200',
    'bg-red-300',
    'bg-red-400',
    'bg-red-500',
    'bg-yellow-50',
    'bg-yellow-100',
    'bg-yellow-200',
    'bg-yellow-300',
    'bg-yellow-400',
    'bg-yellow-500',
    'bg-gray-50',
    'bg-gray-100',
    'bg-gray-200',
    'bg-gray-300',
    'bg-gray-400',
    'bg-gray-500',
    'hover:bg-primary-50',
    'hover:bg-primary-100',
    'hover:bg-primary-200',
    'hover:bg-primary-300',
    'hover:bg-primary-400',
    'hover:bg-primary-500',
    'hover:bg-secondary-50',
    'hover:bg-secondary-100',
    'hover:bg-secondary-200',
    'hover:bg-secondary-300',
    'hover:bg-secondary-400',
    'hover:bg-secondary-500',
    'hover:bg-green-50',
    'hover:bg-green-100',
    'hover:bg-green-200',
    'hover:bg-green-300',
    'hover:bg-green-400',
    'hover:bg-green-500',
    'hover:bg-orange-50',
    'hover:bg-orange-100',
    'hover:bg-orange-200',
    'hover:bg-orange-300',
    'hover:bg-orange-400',
    'hover:bg-orange-500',
    'hover:bg-red-50',
    'hover:bg-red-100',
    'hover:bg-red-200',
    'hover:bg-red-300',
    'hover:bg-red-400',
    'hover:bg-red-500',
    'hover:bg-yellow-50',
    'hover:bg-yellow-100',
    'hover:bg-yellow-200',
    'hover:bg-yellow-300',
    'hover:bg-yellow-400',
    'hover:bg-yellow-500',
    'hover:bg-gray-50',
    'hover:bg-gray-100',
    'hover:bg-gray-200',
    'hover:bg-gray-300',
    'hover:bg-gray-400',
    'hover:bg-gray-500',
    'peer-placeholder-shown:bg-green-100',
    'peer-placeholder-shown:bg-red-100',
    'peer-placeholder-shown:text-green-300',
    'peer-placeholder-shown:text-red-300',
    'focus:bg-primary-50',
    'focus:bg-primary-100',
    'focus:bg-primary-200',
    'focus:bg-primary-300',
    'focus:bg-primary-400',
    'focus:bg-primary-500',
    'focus:bg-secondary-50',
    'focus:bg-secondary-100',
    'focus:bg-secondary-200',
    'focus:bg-secondary-300',
    'focus:bg-secondary-400',
    'focus:bg-secondary-500',
    'focus:bg-green-50',
    'focus:bg-green-100',
    'focus:bg-green-200',
    'focus:bg-green-300',
    'focus:bg-green-400',
    'focus:bg-green-500',
    'focus:bg-orange-50',
    'focus:bg-orange-100',
    'focus:bg-orange-200',
    'focus:bg-orange-300',
    'focus:bg-orange-400',
    'focus:bg-orange-500',
    'focus:bg-red-50',
    'focus:bg-red-100',
    'focus:bg-red-200',
    'focus:bg-red-300',
    'focus:bg-red-400',
    'focus:bg-red-500',
    'focus:bg-yellow-50',
    'focus:bg-yellow-100',
    'focus:bg-yellow-200',
    'focus:bg-yellow-300',
    'focus:bg-yellow-400',
    'focus:bg-yellow-500',
    'focus:bg-gray-50',
    'focus:bg-gray-100',
    'focus:bg-gray-200',
    'focus:bg-gray-300',
    'focus:bg-gray-400',
    'focus:bg-gray-500',
    'text-primary-50',
    'text-primary-100',
    'text-primary-200',
    'text-primary-300',
    'text-primary-400',
    'text-primary-500',
    'text-secondary-50',
    'text-secondary-100',
    'text-secondary-200',
    'text-secondary-300',
    'text-secondary-400',
    'text-secondary-500',
    'text-green-50',
    'text-green-100',
    'text-green-200',
    'text-green-300',
    'text-green-400',
    'text-green-500',
    'text-orange-50',
    'text-orange-100',
    'text-orange-200',
    'text-orange-300',
    'text-orange-400',
    'text-orange-500',
    'text-red-50',
    'text-red-100',
    'text-red-200',
    'text-red-300',
    'text-red-400',
    'text-red-500',
    'text-yellow-50',
    'text-yellow-100',
    'text-yellow-200',
    'text-yellow-300',
    'text-yellow-400',
    'text-yellow-500',
    'text-gray-50',
    'text-gray-100',
    'text-gray-200',
    'text-gray-300',
    'text-gray-400',
    'text-gray-500',
    'text-black',
    'text-white',
    'peer-focus:text-primary-50',
    'peer-focus:text-primary-100',
    'peer-focus:text-primary-200',
    'peer-focus:text-primary-300',
    'peer-focus:text-primary-400',
    'peer-focus:text-primary-500',
    'peer-focus:text-secondary-50',
    'peer-focus:text-secondary-100',
    'peer-focus:text-secondary-200',
    'peer-focus:text-secondary-300',
    'peer-focus:text-secondary-400',
    'peer-focus:text-secondary-500',
    'peer-focus:text-green-50',
    'peer-focus:text-green-100',
    'peer-focus:text-green-200',
    'peer-focus:text-green-300',
    'peer-focus:text-green-400',
    'peer-focus:text-green-500',
    'peer-focus:text-orange-50',
    'peer-focus:text-orange-100',
    'peer-focus:text-orange-200',
    'peer-focus:text-orange-300',
    'peer-focus:text-orange-400',
    'peer-focus:text-orange-500',
    'peer-focus:text-red-50',
    'peer-focus:text-red-100',
    'peer-focus:text-red-200',
    'peer-focus:text-red-300',
    'peer-focus:text-red-400',
    'peer-focus:text-red-500',
    'peer-focus:text-yellow-50',
    'peer-focus:text-yellow-100',
    'peer-focus:text-yellow-200',
    'peer-focus:text-yellow-300',
    'peer-focus:text-yellow-400',
    'peer-focus:text-yellow-500',
    'peer-focus:text-gray-50',
    'peer-focus:text-gray-100',
    'peer-focus:text-gray-200',
    'peer-focus:text-gray-300',
    'peer-focus:text-gray-400',
    'peer-focus:text-gray-500',
    'peer-focus:text-black',
    'peer-focus:text-white',
    'border-primary-50',
    'border-primary-100',
    'border-primary-200',
    'border-primary-300',
    'border-primary-400',
    'border-primary-500',
    'border-secondary-50',
    'border-secondary-100',
    'border-secondary-200',
    'border-secondary-300',
    'border-secondary-400',
    'border-secondary-500',
    'border-green-50',
    'border-green-100',
    'border-green-200',
    'border-green-300',
    'border-green-400',
    'border-green-500',
    'border-orange-50',
    'border-orange-100',
    'border-orange-200',
    'border-orange-300',
    'border-orange-400',
    'border-orange-500',
    'border-red-50',
    'border-red-100',
    'border-red-200',
    'border-red-300',
    'border-red-400',
    'border-red-500',
    'border-yellow-50',
    'border-yellow-100',
    'border-yellow-200',
    'border-yellow-300',
    'border-yellow-400',
    'border-yellow-500',
    'border-gray-50',
    'border-gray-100',
    'border-gray-200',
    'border-gray-300',
    'border-gray-400',
    'border-gray-500',
    'outline-primary-50',
    'outline-primary-100',
    'outline-primary-200',
    'outline-primary-300',
    'outline-primary-400',
    'outline-primary-500',
    'outline-secondary-50',
    'outline-secondary-100',
    'outline-secondary-200',
    'outline-secondary-300',
    'outline-secondary-400',
    'outline-secondary-500',
    'outline-green-50',
    'outline-green-100',
    'outline-green-200',
    'outline-green-300',
    'outline-green-400',
    'outline-green-500',
    'outline-orange-50',
    'outline-orange-100',
    'outline-orange-200',
    'outline-orange-300',
    'outline-orange-400',
    'outline-orange-500',
    'outline-red-50',
    'outline-red-100',
    'outline-red-200',
    'outline-red-300',
    'outline-red-400',
    'outline-red-500',
    'outline-yellow-50',
    'outline-yellow-100',
    'outline-yellow-200',
    'outline-yellow-300',
    'outline-yellow-400',
    'outline-yellow-500',
    'outline-gray-50',
    'outline-gray-100',
    'outline-gray-200',
    'outline-gray-300',
    'outline-gray-400',
    'outline-gray-500',
    'focus:border-primary-50',
    'focus:border-primary-100',
    'focus:border-primary-200',
    'focus:border-primary-300',
    'focus:border-primary-400',
    'focus:border-primary-500',
    'focus:border-secondary-50',
    'focus:border-secondary-100',
    'focus:border-secondary-200',
    'focus:border-secondary-300',
    'focus:border-secondary-400',
    'focus:border-secondary-500',
    'focus:border-green-50',
    'focus:border-green-100',
    'focus:border-green-200',
    'focus:border-green-300',
    'focus:border-green-400',
    'focus:border-green-500',
    'focus:border-orange-50',
    'focus:border-orange-100',
    'focus:border-orange-200',
    'focus:border-orange-300',
    'focus:border-orange-400',
    'focus:border-orange-500',
    'focus:border-red-50',
    'focus:border-red-100',
    'focus:border-red-200',
    'focus:border-red-300',
    'focus:border-red-400',
    'focus:border-red-500',
    'focus:border-yellow-50',
    'focus:border-yellow-100',
    'focus:border-yellow-200',
    'focus:border-yellow-300',
    'focus:border-yellow-400',
    'focus:border-yellow-500',
    'focus:border-gray-50',
    'focus:border-gray-100',
    'focus:border-gray-200',
    'focus:border-gray-300',
    'focus:border-gray-400',
    'focus:border-gray-500',
    'focus:outline-primary-50',
    'focus:outline-primary-100',
    'focus:outline-primary-200',
    'focus:outline-primary-300',
    'focus:outline-primary-400',
    'focus:outline-primary-500',
    'focus:outline-secondary-50',
    'focus:outline-secondary-100',
    'focus:outline-secondary-200',
    'focus:outline-secondary-300',
    'focus:outline-secondary-400',
    'focus:outline-secondary-500',
    'focus:outline-green-50',
    'focus:outline-green-100',
    'focus:outline-green-200',
    'focus:outline-green-300',
    'focus:outline-green-400',
    'focus:outline-green-500',
    'focus:outline-orange-50',
    'focus:outline-orange-100',
    'focus:outline-orange-200',
    'focus:outline-orange-300',
    'focus:outline-orange-400',
    'focus:outline-orange-500',
    'focus:outline-red-50',
    'focus:outline-red-100',
    'focus:outline-red-200',
    'focus:outline-red-300',
    'focus:outline-red-400',
    'focus:outline-red-500',
    'focus:outline-yellow-50',
    'focus:outline-yellow-100',
    'focus:outline-yellow-200',
    'focus:outline-yellow-300',
    'focus:outline-yellow-400',
    'focus:outline-yellow-500',
    'focus:outline-gray-50',
    'focus:outline-gray-100',
    'focus:outline-gray-200',
    'focus:outline-gray-300',
    'focus:outline-gray-400',
    'focus:outline-gray-500',
]